...
export default function TrelloActionsUser({ TeamID, UserID, BoardID, sendModal }: Props) {
                                    ...
                                    <tr key={action.ActionID} className="flex flex-row w-full bg-gray-300 px-3 py-1 rounded-2xl justify-between gap-25">
                                        ...
                                        </td>
                                        {
                                            action.oldData === null ? (
                                                ...
                                                <td onClick={() => toggleModalHandler(action.CardID)} className="px-2 items-center max-w-60 text-center hover:cursor-pointer bg-green-300 hover:bg-green-500 rounded-2xl">
                                                    Current Card</td>
                                            )
                                        }
    ...
    async function fetchTrelloCards() {
        ...
        const res = await fetch(`/api/trello/fetchtrellocards?BoardID=${BoardID}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'authorization': `Bearer ${access_token}`
    ...
    async function fetchTrelloLists() {
        ...
        const res = await fetch(`/api/trello/fetchtrellolists?BoardID=${BoardID}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'authorization': `Bearer ${access_token}`
        ...
        if (!res.ok) {
            console.log("Error fetching trello lists:", res)
        }
