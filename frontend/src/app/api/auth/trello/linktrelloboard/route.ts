...
export async function GET(req: NextRequest) {
    try {
        ...
        const currentBoard = params.get("currentBoard");


        const response = await fetch(`${process.env.API_URL}/router/linktrelloboard?TeamID=${TeamID}&url=${trelloBoardUrl}&currentBoard=${currentBoard}`, {
            method: 'GET',
            headers: {
